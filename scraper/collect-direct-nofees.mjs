...
//
// ИТОГ ПРОГОНА 02.09.2026 — ОБЩИЙ СБОРЩИК ЭТИ САЙТЫ НЕ БЕРЁТ. Прогнаны первые
// четыре слага, у всех результат негодный: у sp-jain «программами» приехали
// 37 профилей преподавателей («Vanita Bhoola»), у anglo-american — «Thank you»
// и заголовки новостей, у inti — названия факультетов, а не программ.
// ЦЕН НЕ НАЙДЕНО НИ ОДНОЙ НИ У КОГО. Выгрузки откачены, в каталог не поехало.
//
// Причина: kompas-collect-direct ищет карточки курсов по общей разметке, а у этих
// вузов каталог программ лежит на своей вёрстке. Нужен парсер НА ДОМЕН —
// ровно то, во что план оценивал A1 в $8–15, а не один общий проход.
// Прежде чем запускать снова, напиши разбор под конкретный домен.
//
