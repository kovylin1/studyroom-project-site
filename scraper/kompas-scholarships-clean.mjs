...
async function main() {
  ...
  addCase('__kompas__', 'Каталог целиком', 'kompas_scholarship_parse_junk', 'warning',
    `Просмотр выборки собранных стипендий показал брак разбора: у ${ever.length} вузов из ${everBefore} записей отброшено ${everRemoved} — предложения вместо названий («We offer scholarships»), обрывки абзацев («average financial assistance award of $8,092»), обобщённые разделы («External Scholarships»). По происхождению затронутых вузов: ${Object.entries(everOrigin).map(([k, v]) => `${k} ${v.removed}/${v.records}`).join(', ')} — заголовки страницы-раздела грязнее отдельных страниц, как и ожидалось. Имён почищено от склейки с описанием ${everRenamed}. В выгрузках осталось ${kept} записей. Правила разбора ужесточены и покрыты тестами; суммы НЕ чинились — «до …» и «/мес» восстанавливаются только повторным обходом сети.`,
