...
// как число остаётся верной в новой валюте). Каждая — с обоснованием.
const RELABEL = [
  {
    ...
    reason: 'BHMS — швейцарская школа гостеприимства (Люцерн), прайсит в CHF. '
      ...
      + 'EUR/CHF ≈ паритет, поэтому сумма как число остаётся верной под новой валютой.',
  },
];
...
async function main() {
  ...
  cases.push({
    ...
    severity: 'info',
    detail: `79 расхождений валюты сверены со страной вуза. Настоящий баг один — bhms EUR→CHF (исправлен). Остальные ${REVIEWED_KEEP.length} проверенных вузов оставлены как есть: `
