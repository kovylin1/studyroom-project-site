...
const RELABEL = [
  // bhms НЕ относим: суммы — франки, но CHF нет в enum валют схемы. Относка на CHF
  // сделала бы карточку невалидной, а вписать её честно нельзя без фичи (schema +
  // символьная карта + конвертация в KZT). Оставляем EUR (≈ паритет к CHF, ~5%),
  // а настоящую природу цены выносим в кейс-решение владельцу (см. REVIEWED_KEEP).
];
...
const REVIEWED_KEEP = [
  { slug: 'bhms', why: 'Switzerland: суммы — франки (CHF), но enum валют схемы CHF не поддерживает. Оставлен EUR (≈ паритет, ~5%). Чтобы отразить CHF честно — нужна фича: CHF в schema + символ + курс KZT в static.ts/CompareBody. РЕШЕНИЕ ВЛАДЕЛЬЦА.' },
...
async function main() {
  ...
  cases.push({
    ...
    severity: 'info',
    detail: `79 расхождений валюты сверены со страной вуза. Авто-относок валюты: ${backup.length}. `
      + `Единственный настоящий мислейбл — bhms (франки под меткой EUR), но enum валют схемы не поддерживает CHF, поэтому оставлен EUR до решения владельца о поддержке CHF. `
      + `Проверенные вузы, оставленные как есть (${REVIEWED_KEEP.length}): `
