...
- [ ] **165 карточек без офсайта — нужен поиск, которого у нас нет.** Wikidata их не знает, своих
  ...
  платно), либо руками.
- [x] ~~**Суммы стипендий в собранных выгрузках чинятся только повторным обходом.**~~ **Прогон
  `kompas-collect-scholarships.mjs --refetch` запущен 2026-08-10.** Правила разбора («до $1,000»
  вместо `$1,000`, «$60 per month» → `/мес`) починены и покрыты тестами ещё в сессии 6 — перебор
  применяет их к выгрузкам. Живой каталог перебор не трогает: он пишет в
  `sources/kompas/extracts/scholarships`, каталог правит отдельный `kompas-scholarships-apply.mjs`.
