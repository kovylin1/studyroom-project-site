...
- [x] **Применить рабочую копию в живой каталог — СДЕЛАНО 23.08 (задача 3.1), запись ниже устарела.**
  Замер 09.09: обе копии сходятся — 1076 карточек, 124 265 программ, 91 242 цены, «только в копии» 0, «только в живом» 0. Цифры ниже («вырастет с 91 213 до ~106 000») относятся к состоянию до переноса.
...
- [x] **53 привязки выгрузок QS — ЗАКРЫТО, проверено замером 09.09.**
  Из 512 выгрузок QS без `catalogSlug` осталось 5, и все пять — мусор портала, а не пробел: `testing-december-testing-purpose` (тестовая запись) и `indian-internal-applications` (служебная) помечены `notAnInstitution`; у `american-collegiate-washington-dc` и `durham-college` ноль программ; остаётся `oxford-international-education-group-north-america-pathway-programs` (4 программы). Привязку закрыли прогоны `kompas-qs-relink-cat1.mjs` и `-forced.mjs`. Прежняя запись: Схожесть имени
...
- [x] **17 кейсов `kompas_fee_amount_fx` — ЗАКРЫТО замером 09.09, осталось 2.**
  `scraper/kompas-fx-amount-check.mjs` приводит обе суммы к тенге справочным курсом и учитывает варианты цены В ИХ СОБСТВЕННОЙ валюте. Пар с разной валютой 254, после пересчёта сходится 252. Остаются 2 строки Hult — многокампусная школа, QS прайсит разные кампусы (78 000 USD против 184 300 AED). Разбор 15.08 считал 17 кейсов до переприменения цен. Прежняя запись: Сверка
...
- [x] **59 карточек `kompas_no_extract` — снято тем же замером 09.09** (см. пункт про привязки выше). Разбор карточек без выгрузки теперь живёт в `scraper/kompas-sourceless-cards.mjs`. Прежняя запись: это тот же
  вопрос, что и 53 кейса выше, только со стороны карточки.
- [x] **Брак разбора кампусов у коллекторов — в каталог не течёт, сторож стоит.**
  Замер 09.09: у всех шести названных карточек (`ema`, `englishpath`, `gbs-dubai`, `gbs-malta`, `global-banking-school`, `schiller-international-university`) кампусы чистые, длинных строк 0. `kompas-backfill-campuses.mjs` отбивает абзацы разрядом `prose`; открытым остался 1 предупреждающий кейс — `florida-international`. Чинить коллектор имеет смысл только вместе со следующим обходом. Прежняя запись: У ema, englishpath, gbs-dubai, gbs-malta,
...
### Ссылки программ ведут на чужой вуз (замер 2026-09-09)

- [ ] **366 программ ссылаются на сайт ДРУГОГО вуза из каталога — нужно решение владельца.**
  `scraper/kompas-programurl-check.mjs` сравнивает домен `programUrl` с `officialUrl` карточки
  и узнаёт чужой вуз по домену. Три пары-однофамильца:
  **nottingham → ntu.ac.uk (Nottingham Trent) 199 программ**, из них 189 `catalog-only`, у всех 199 стоит цена;
  **bristol → courses.uwe.ac.uk (University of the West of England) 118**, все с ценой, ни одна не `catalog-only`;
  **liverpool → xjtlu.edu.cn (Xi'an Jiaotong-Liverpool) 49**, все `catalog-only`, цен нет.
  Это не опечатка в ссылке: у программ стоит статус «агрегатор не подтверждает», то есть
  их, скорее всего, собрали с сайта чужого вуза и положили не в ту карточку. Посетитель со
  страницы Ноттингема уходит в Nottingham Trent.
  **Вопрос владельцу:** снимать программы целиком или оставлять, сняв ссылку? Снятие ссылки
  дешевле и безопаснее (правило `program-link.ts` уже говорит «офсайта нет — ссылки нет вовсе»),
  но оставляет в карточке чужой состав. Отчёт — `sources/kompas/PROGRAMURL-CHECK.md`.
  Заодно: 220 ссылок на домены, которых нет ни у одной карточки — их никто не проверял.
