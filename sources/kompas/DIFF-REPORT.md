...
**Дата:** 2026-07-25 · сети нет, каталог не тронут (только чтение).
...
| **Сверено с источником** | **463** |
| Сверить не с чем: источник за логином | 227 (QS, IAPro) |
| Сверить не с чем: источник без программ | 11 (Navitas, CATS) |
| Сверить не с чем: источник готов, выгрузки по вузу нет | 7 |
| Программ в каталоге (сверенные вузы) | 71559 |
| Программ у источников (объединение) | 51745 |
| Совпало названий | 49938 (из них по написанию: 356) |
| Есть в каталоге, нет у источника | 21621 |
| Есть у источника, нет в каталоге | 1807 |
| Цена расходится | 1723 |
| **Валюта расходится** | **77** |
| У источника цена есть, в каталоге нет | 1720 |
| Кампусы источника, которых нет в карточке | 101 |

Кейсов в панель: **1264** — kompas_programs_extra 333, kompas_campus_missing 57, kompas_fee_mismatch 731, kompas_fee_absent 48, kompas_programs_missing 58, kompas_fee_currency 10, kompas_fee_mismatch_rest 18, kompas_no_extract 7, kompas_source_blocked 1, kompas_source_empty 1.
...
| University of Lancashire (`university-of-lancashire`) | edvoy | 452 | 330 | 330 | 122 | 0 | 250 | 0 |
| University of Glasgow (`glasgow`) | kaplan+oxford-international+edvoy | 648 | 531 | 531 | 117 | 0 | 215 | 0 |
...
| Arizona State University (`arizona-state`) | kaplan+edvoy | 875 | 639 | 632 | 243 | 7 | 88 | 0 |
| Royal Holloway, University of London (`royal-holloway`) | studygroup+edvoy | 496 | 11 | 11 | 485 | 0 | 0 | 0 |
| Victoria University of Wellington (`victoria-wellington`) | edvoy | 883 | 445 | 444 | 439 | 1 | 15 | 0 |
...
| Edinburgh Napier University (`edinburgh-napier`) | edvoy | 221 | 148 | 148 | 73 | 0 | 119 | 0 |
...
| Massey University (`massey`) | edvoy | 670 | 284 | 284 | 386 | 0 | 0 | 0 |
...
| University of Westminster (`westminster`) | kaplan+edvoy | 331 | 312 | 312 | 19 | 0 | 91 | 0 |
| Kaplan ANZ (`kaplan-anz`) | edvoy | 318 | 28 | 28 | 290 | 0 | 0 | 0 |
| Arden University (`arden`) | edvoy+iapro | 126 | 28 | 28 | 98 | 0 | 0 | 19 |
| University of Nottingham (`nottingham`) | kaplan+edvoy | 671 | 399 | 397 | 274 | 2 | 3 | 0 |
| Aston University (`aston`) | edvoy | 269 | 194 | 194 | 75 | 0 | 60 | 0 |
...
| Pace University (`pace`) | kaplan+edvoy | 285 | 102 | 102 | 183 | 0 | 15 | 0 |
| The University of Sydney (`sydney`) | edvoy | 460 | 435 | 421 | 39 | 14 | 57 | 0 |
...
| Griffith University (`griffith`) | edvoy | 963 | 915 | 913 | 50 | 2 | 44 | 0 |
